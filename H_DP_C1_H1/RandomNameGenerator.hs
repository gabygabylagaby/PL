...
totalStudents archivo = do
    ...
    let lineas = lines contenido 
        estudiantes = map parseLine $ filter (\linea -> not (null linea) && head linea /= '-') lineas  
    return estudiantes
...
assignSchoolHouse :: SchoolHouseCount -> IO (String, SchoolHouseCount)
assignSchoolHouse houseCount = do
    let minCount = minimum [griffindorCount houseCount, hufflepuffCount houseCount, ravenclawCount houseCount, slytherinCount houseCount]
    if griffindorCount houseCount == minCount
        then return ("Griffindor", houseCount { griffindorCount = griffindorCount houseCount + 1 })
    else if hufflepuffCount houseCount == minCount
        then return ("Hufflepuff", houseCount { hufflepuffCount = hufflepuffCount houseCount + 1 })
    else if ravenclawCount houseCount == minCount
        then return ("Ravenclaw", houseCount { ravenclawCount = ravenclawCount houseCount + 1 })
    else
        return ("Slytherin", houseCount { slytherinCount = slytherinCount houseCount + 1 })
...
studentsPerHouse :: Int -> Int
studentsPerHouse totalStudents = totalStudents `div` numberOfConstructors
...
main = do
    students <- totalStudents "/home/gabysan/Desktop/Lenguaje de Programacion/H_DP_C1/list.txt"
    let totalStudentsCount = length students
        initialHouseCount = SchoolHouseCount 0 0 0 0 
        maxStudentsPerHouse = studentsPerHouse totalStudentsCount
    putStrLn $ "El número total de casas es: " ++ show numberOfConstructors
    putStrLn $ "El número total de estudiantes es: " ++ show totalStudentsCount
    putStrLn $ "El número de estudiantes por casa es: " ++ show (studentsPerHouse totalStudentsCount)
    finalHouseCount <- studentsList students initialHouseCount
    putStrLn "Cantidad de estudiantes asignados a cada casa:"
    putStrLn $ "Griffindor: " ++ show (griffindorCount finalHouseCount)
    putStrLn $ "Hufflepuff: " ++ show (hufflepuffCount finalHouseCount)
